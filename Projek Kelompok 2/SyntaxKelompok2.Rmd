...
Berdasarkan uji KS tersebut, didapat *p-value* sebesar 0.2836 yang lebih dari taraf nyata 5% sehingga tak tolak $H_0$ dan menandakan bahwa sisaan menyebar normal. Hal ini sesuai dengan hasil eksplorasi menggunakan plot kuantil-kuantil normal.
...
Berdasarkan uji Ljung-Box tersebut, didapat *p-value* sebesar 0.01541 yang lebih kecil dari taraf nyata 5% sehingga tolak $H_0$ dan menandakan bahwa sisaan tidak saling bebas.
...
### Penanganan sisaan tidak saling bebas/ada autokorelasi
```{r}
library(rcompanion)
train.diff2<-rcompanion::transformTukey(train.diff)
```
...
Identifikasi model menggunakan plot EACF dilakukan dengan melihat ujung segitiga pada pola segitiga nol. Dalam hal ini model tentatif yang terbentuk adalah ARIMA(1,2,2) ARIMA(1,2,3), ARIMA(3,2,2)
...
```{r}
model1.da2=Arima(train.diff2, order=c(2,2,2),method="ML")
summary(model1.da2) #AIC=739.84
lmtest::coeftest(model1.da2) #seluruh parameter signifikan
```

#### ARIMA(1,2,2)

```{r}
model2.da2=Arima(train.diff2, order=c(1,2,2),method="ML")
summary(model2.da2) #AIC=757.1
lmtest::coeftest(model2.da2) #seluruh parameter signifikan
```

#### ARIMA(1,2,3)

```{r}
model3.da2=Arima(train.diff2, order=c(1,2,3),method="ML")
summary(model3.da2) #AIC=751.85
lmtest::coeftest(model3.da2) #MA2 tidak signifikan
```

#### ARIMA(3,2,2)

```{r}
model4.da2=Arima(train.diff2, order=c(3,2,2),method="ML")
summary(model4.da2) #733.5
lmtest::coeftest(model4.da2) #semua signifikan
```

```{r}
# Membuat vektor yang berisi nama model
model_names <- c("model1.da2", "model2.da2", "model3.da2", "model4.da2")
...
Berdasarkan pendugaan parameter di atas, nilai AIC terkecil dimiliki oleh model ARIMA(3,2,2) dan parameter model ARIMA(3,2,2) juga seluruhnya signifikan sehingga model yang dipilih adalah model ARIMA(3,2,2).
...
```{r}
#Eksplorasi 
sisaan.da2 <- model4.da2$residuals 
par(mfrow=c(2,2)) 
qqnorm(sisaan.da2) 
qqline(sisaan.da2, col = "blue", lwd = 2) 
plot(c(1:length(sisaan.da2)),sisaan.da2) 
acf(sisaan.da2) 
pacf(sisaan.da2) 
par(mfrow = c(1,1))
```

Berdasarkan plot kuantil-kuantil normal, secara eksplorasi ditunjukkan sisaan  menyebar normal ditandai dengan titik titik yang cenderung tidak mengikuti garis $45^{\circ}$. Kemudian dapat dilihat juga lebar pita sisaan yang cenderung sama menandakan bahwa sisaan memiliki ragam yang homogen. Plot ACF dan PACF sisaan ARIMA(3,2,2) tidak signifikan pada 20 lag awal yang menandakan sisaan saling bebas. Kondisi ini akan diuji lebih lanjut dengan uji formal.

#### Uji Formal

```{r}
#1) Sisaan Menyebar Normal 
ks.test(sisaan.da2,"pnorm")  #tak tolak H0 > sisaan menyebar normal
```
...
Berdasarkan uji KS tersebut, didapat *p-value* sebesar 0.07541 yang lebih dari taraf nyata 5% sehingga tolak $H_0$ dan menandakan bahwa sisaan menyebar normal. Hal ini sesuai dengan hasil eksplorasi menggunakan plot kuantil-kuantil normal.

```{r}
#2) Sisaan saling bebas/tidak ada autokorelasi 
Box.test(sisaan.da2, type = "Ljung")  #tak tolak H0 > sisaan saling bebas
```
...
Berdasarkan uji Ljung-Box tersebut, didapat *p-value* sebesar 0.4166 yang lebih besar dari taraf nyata 5% sehingga tak tolak $H_0$ dan menandakan bahwa sisaan saling bebas.

```{r}
#3) Sisaan homogen 
Box.test((sisaan.da2)^2, type = "Ljung")  #tak tolak H0 > sisaan homogen
```
...
Berdasarkan uji Ljung-Box terhadap sisaan kuadrat tersebut, didapat *p-value* sebesar 0.6698 yang lebih dari taraf nyata 5% sehingga tak tolak $H_0$ dan menandakan bahwa ragam sisaan homogen.

```{r}
#4) Nilai tengah sisaan sama dengan nol 
t.test(sisaan.da2, mu = 0, conf.level = 0.95)  #tak tolak h0 > nilai tengah sisaan sama dengan 0
```
...
Berdasarkan uji-ttersebut, didapat *p-value* sebesar 0.9675 yang lebih besar dari taraf nyata 5% sehingga tak tolak $H_0$ dan menandakan bahwa nilai tengah sisaan sama dengan nol. Hal ini berbeda dengan eksplorasi.
